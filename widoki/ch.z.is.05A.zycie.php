...
$table = $section->addTable('czesci-tablica');
    $table->addRow($wWie);
        ...
        $table->addCell($dlPoleTytulDlugi,array('valign' => 'center','bgcolor'=>$formatKropki['color']))->addText('Chrześcijański tryb życia',array('name'=>'Calibri','size'=>10,'color'=>'FFFFFF','bold'=>true,'allCaps'=>true),$akapitTytul);
        ...
        $table->addRow($wWie);
            ...
            if ($punkt['pomocnik'] != '') {   //zborowe studium Biblii
                ...
                $punktZycie=$table->addCell($dlPoleTytulKrotki,array('valign' => 'center'))->addTextRun();
                    ...
                    $tekstKonca=" min lub mniej)";
//                    if ($punkt === end($zebranie->get_punkty_skarby())) $tekstKonca=" min lub mniej)";
                    $punktZycie->addText("(".$punkt['czas'].$tekstKonca,$fontTytulMaly,$akapitTytul);                
...
//Powtórka i zapowiedź następnego zebrania
    $table->addRow($wWie);
        ...
        $punktSkarby=$table->addCell($dlPoleTytulKrotki,array('valign' => 'center'))->addTextRun();
            $punktSkarby->addText(' ', $formatKropki);
            $punktSkarby->addText('Uwagi końcowe ',$fontTytul,$akapitTytul);
            $punktSkarby->addText("(3 min lub mniej)",$fontTytulMaly,$akapitTytul);
        $table->addCell($dlPoleProwadzacy,array('valign' => 'center'))->addText("",$fontRola,$akapitRola);
